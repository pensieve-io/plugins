...
installs an upload-only credential for this client. It does not enable capture,
change company contribution settings, read old chats or inspect host login tokens.
...
2. Open or show the helper's `verification_url` in the user's normal browser.
   ...
   company knowledge sharing. Full chats and published knowledge have separate
   visibility. Do not approve on the user's behalf or silently change their settings.
